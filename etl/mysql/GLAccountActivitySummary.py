...
try:
    ...
    cursor.execute("{call sproc300758_11728751_1980869(?,?,?,?)}", PCNList,PeriodOffset,ExcludePeriod13,ExcludePeriodAdjustments)
    ...
    rowcount=cursor2.execute("{call Plex.gl_account_activity_summary_delete_current_period}").rowcount
    # rowcount=cursor2.execute("{call Scratch.account_period_balance_delete_period_range}").rowcount
    # https://github.com/mkleehammer/pyodbc/wiki/Cursor
    # The return value is always the cursor itself:
    print_to_stdout(f"call Plex.gl_account_activity_summary_delete_current_period - rowcount={rowcount}")
    print_to_stdout(f"call Plex.gl_account_activity_summary_delete_current_period - messages={cursor2.messages}")
    cursor2.commit()
    ...
    im2='''insert into Plex.GL_Account_Activity_Summary(pcn,period,account_no,debit,credit,net)
    values (?,?,?,?,?,?)''' 
    ...
    cursor3.callproc('gl_account_activity_summary_delete_current_period', [])
    # rowcount=cursor2.execute(txt.format(dellist = params)).rowcount
    print_to_stdout(f"call Plex.gl_account_activity_summary_delete_current_period() - rowcount={cursor3.rowcount}")
